-- ORBI RESET SCHEMA (FULL REBUILD)
-- Generated from the master schema with settlement_lifecycle fixes merged in.
-- WARNING: THIS SCRIPT WILL DROP AND RECREATE OBJECTS.
...
DROP TABLE IF EXISTS public.provider_config_versions CASCADE;
...
DROP TABLE IF EXISTS public.api_gateway_quarantines CASCADE;
DROP TABLE IF EXISTS public.api_gateway_security_events CASCADE;
...
DROP TABLE IF EXISTS public.fx_reconciliation_events CASCADE;
DROP TABLE IF EXISTS public.fx_treasury_exposure_limits CASCADE;
DROP TABLE IF EXISTS public.fx_provider_health CASCADE;
...
DROP TABLE IF EXISTS public.organization_limit_configs CASCADE;
...
DROP TABLE IF EXISTS public.merchant_settlement_reports CASCADE;
DROP TABLE IF EXISTS public.service_commissions CASCADE;
DROP TABLE IF EXISTS public.service_commission_disputes CASCADE;
...
DROP TABLE IF EXISTS public.agent_float_controls CASCADE;
...
-- V93.0: Includes settlement_lifecycle creation and corrected settlement indexes.
...
SELECT
  ...
  'ACTIVE',
  jsonb_build_object(
    'seeded_by', 'database/reset_schema.sql',
