...
3. The ADMIN can then invite other users using their Orbi email address (`POST /v1/enterprise/users/invite`). The invited user will instantly receive a real-time push notification and inherit the `organization_id` with an `EMPLOYEE` or `FINANCE` role.
