...
*   **Deployment**: Production setup and servicing are maintained in the ORBI Core repository.
...
ORBI uses the Brevo API to send automated emails (OTP, confirmations, etc.). To use Brevo as the email provider, follow these steps:

### 9.1 Brevo Setup
1.  **Create an Account**: Sign up for a Brevo account at brevo.com.
2.  **Generate API Key**:
    *   Go to your Brevo account settings -> SMTP & API.
    *   Click on "API Keys" and generate a new v3 API key.
...
### 9.3 Usage
The `EmailService` class automatically initializes the SMTP transporter using these variables. It can be imported and used in any service:
```typescript
